...
export default function CreateTransaction() { 
    ...
    const formattedExpiryDate = `${expiryDate.getDate()} ${expiryDate.toLocaleString('default', { month: 'short' })},${expiryDate.getFullYear()}`;
    ...
    const isCategoryBlocked = (categoryText: string) => {
        return activeLimits.some(limit => limit.category === categoryText);
    };
    const handleCreateLimit = async () => {
        try {
            ...
            const response = await fetch(`${API_URI}/createlimit`, {
                method: 'POST',
                headers: {
    ...
    return(
        <View style={styles.container}>
            ...
            <View style={styles.content}>
                <Text style={{ color: 'white', fontFamily: 'RobotoMono-Regular', fontSize: 20 }}>Create Limit</Text>
                <Text style={{ color: '#A6A6A6', fontFamily: 'RobotoMono-Regular', fontSize: 14 }}>—select category to set limit {'\n'}for a category</Text>
                <View style={styles.categoryContainer}>
                    {category.map((cat, index) => {
                        const blocked = isCategoryBlocked(cat.text);
                        return (
                        <TouchableOpacity key={index} style={[styles.categoryWrapper, blocked && { opacity: 0.4 }]} onPress={() => { if (!blocked) {setCategoryIcon(cat.icon); setCategoryText(cat.text);}}} disabled={blocked}><Image source={cat.icon} style={styles.category}/></TouchableOpacity>
                        );
                    })}
                </View>
            </View>
            ...
            <Animated.View style={[styles.inputContainer, { transform: [{ translateY: shift }] }]}>
                ...
                <View style={{ alignItems: 'flex-end', flex: 3.2, justifyContent: 'flex-start' }}>
                    <Text style={{ fontFamily: 'RobotoMono-Regular', fontSize: 12, color: 'white' }}>expiry date</Text>
                    <TouchableOpacity onPress={() => setShowDatePicker(true)}>
                        <Text style={{ fontFamily: 'RobotoMono-Regular', fontSize: 12, color: '#1A73E9' }}>{formattedExpiryDate}</Text>
                    </TouchableOpacity>
                    {showDatePicker && (
                        <DateTimePicker value={expiryDate} mode='date' display='default' onChange={onChangeDate} minimumDate={new Date()} />
                    )}
                </View>
            </Animated.View>
            <TouchableOpacity style={styles.create} onPress={handleCreateLimit}><Text style={{ fontFamily: 'RobotoMono-Regular', fontSize: 14, color: 'white' }}>create</Text></TouchableOpacity>
        </View>
    );
}
